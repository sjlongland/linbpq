...
# Default-Stop:      0 1 6
# Short-Description: LinBPQ NET/ROM and AX.25 Node
# Description:       LinBPQ is a packet radio switch for linking a Linux
#                    PC to an amateur radio AX.25 packet network.
### END INIT INFO
...
DESC="linbpq AX.25 and NET/ROM switch"
NAME=linbpq
DAEMON=/usr/bin/linbpq
LINBPQ_HOME=/var/lib/linbpq
DAEMON_ARGS=""
...
do_stop()
{
	...
	start-stop-daemon --chdir ${LINBPQ_HOME} --stop --quiet --retry=TERM/30/KILL/5 --pidfile $PIDFILE --name $NAME
	RETVAL="$?"
	[ "$RETVAL" = 2 ] && return 2
	...
	# sleep for some time.
	start-stop-daemon --chdir ${LINBPQ_HOME} --stop --quiet --oknodo --retry=0/30/KILL/5 --exec $DAEMON
	[ "$?" = 2 ] && return 2
